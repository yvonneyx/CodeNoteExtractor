...
const __dirname =
  '/Users/banxuan/Documents/code-note-extractor/packages/site/scripts';
...
const reportFolderRoot = path.resolve(
  __dirname,
  '..',
  path.join('support', 'api'),
);
const reportTempFolderRoot = path.resolve(reportFolderRoot, 'temp');
const ignorePackages = new Set<string>(['site']);
...
async function runApiExtractor() {
  ...
  for (const pkg of packages) {
    ...
    const reportTempFilePath = path.join(
      reportTempFolderRoot,
      `${name}.api.md`,
    );
    ...
    if (fs.existsSync(packageJsonFullPath)) {
      const packageJson = JSON.parse(
        fs.readFileSync(packageJsonFullPath, 'utf8'),
      );
